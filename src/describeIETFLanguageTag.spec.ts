...
describe('f', () => {
  ...
  describe('Language subtag plus Script subtag', () => {
    test('zh-Hant', () =>
      expect(f('zh-Hant')).toBe('Chinese, written in Han (Traditional variant) script'));
    test('zh-Hans', () =>
      expect(f('zh-Hans')).toBe('Chinese, written in Han (Simplified variant) script'));
  ...
  describe('Extended language subtags and their primary language subtag counterparts', () => {
    ...
    test('zh-yue-HK', () =>
      expect(f('zh-yue-HK')).toBe('Yue Chinese / Cantonese, as used in Hong Kong'));
  ...
  describe('Language-Script-Region', () => {
    ...
    test('sr-Latn-RS', () =>
      expect(f('sr-Latn-RS')).toBe('Serbian, written in Latin script, as used in Serbia'));
  });

  describe('Language-Variant', () => {
    test('sl-rozaj', () => expect(f('sl-rozaj')).toBe('Resian variant of Slovenian'));
    test('sl-rozaj-biske', () =>
      expect(f('sl-rozaj-biske')).toBe(
  ...
  describe('Language-Region-Variant', () => {
    ...
    test('sl-IT-nedis', () =>
      expect(f('sl-IT-nedis')).toBe('Natisone dialect of Slovenian, as used in Italy'));
  });

  describe('Language-Script-Region-Variant', () => {
    test('hy-Latn-IT-arevela', () =>
  ...
  describe('Language-Region', () => {
    ...
    test('es-419', () =>
      expect(f('es-419')).toBe('Spanish / Castilian, as used in Latin America and the Caribbean'));
  });
  ...
  describe('Private use registry values', () => {
    ...
    test('de-Qaaa', () =>
      expect(f('de-Qaaa')).toBe('German, written in private use script ‘Qaaa’ script'));
    test('sr-Latn-QM', () =>
