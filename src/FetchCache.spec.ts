...
describe('FetchCache', () => {
  ...
  describe('TTL handling', () => {
    it('returns cached response, evicts response after TTL, and makes a second request', async () => {
      ...
      const cache = new FetchCache({ fetch, ttl: () => 10000 });
      ...
      expect(internalCache.getItem('url')).toBeUndefined();
    ...
    it('evicts items after different TTLs depending on response status/error', async () => {
      ...
      const ttlFunction: TTLFunction<any> = cachedValue => {
      ...
      dateNowSpy.mockReturnValue(0);
      const cache = new FetchCache({ fetch, ttl: ttlFunction });

      const promise200 = cache.fetch('200');
      expect(cache.cache.peekItem('200')).toMatchObject({ expireAfterTimestamp: 5000 });
      if (!resolveWith200) {
        throw new Error(
      ...
      expect(cache.cache.peekItem('200')).toMatchObject({ expireAfterTimestamp: 6000 });

      const promise404 = cache.fetch('404');
      expect(cache.cache.peekItem('404')).toMatchObject({ expireAfterTimestamp: 5000 });
      if (!resolveWith404) {
        throw new Error(
      ...
      expect(cache.cache.peekItem('404')).toMatchObject({ expireAfterTimestamp: 7000 });

      const promiseWithError = cache.fetch('error');
      expect(cache.cache.peekItem('error')).toMatchObject({ expireAfterTimestamp: 5000 });
      if (!rejectWithError) {
        throw new Error(
      ...
      expect(cache.cache.peekItem('error')).toMatchObject({ expireAfterTimestamp: 10000 });
    });
  });
  ...
  describe('compatibility with 3rd party implementations', () => {
    describe('with `node-fetch` NPM package', () => {
      it('fetches from a local HTTP test server', async () => {
        ...
        const cache = new FetchCache({ fetch: nodeFetch });
    ...
    describe('with `normalize-url` package', () => {
      it('returns the cached response when requesting variants of the same normalized URL', async () => {
        ...
        const cache = new FetchCache({ fetch, normalizeURL });
